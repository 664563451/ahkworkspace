...
; 5. 按住鼠标侧键1可以连点左键用于拾取。松开释放。
; 6. 可以修改右键为跑马，按住右键触发。
; 7. F4赌博，鼠标放在要赌博的位置然后按F4
; 8. F7一键扔背包装备，打开背包，扔掉除第一列的物品。
; 9. F8 一键分解除第一列的物品。打开铁匠后按F8
; 10. F11 选择宝石，按F11自动升级宝石
; 11. F12 按一下大号开启大米小号接受（需在秘境选择界面）。连按两下小号传送到大号。连按三下小号回城。
